...
class Game:
    ...
    def menu_selection(self, event):
        ...
        if selected_mode == "Fly Through All The Holidays":
            for i in ["Halloween", "Thanksgiving", "Christmas"]:
                ...
                self.start_game(i)
                self.game_running()
                score.set_through_seasons_score()
            score.set_score(score.get_through_seasons_score())
    ...
    def start_game(self, holiday):
        ...
        self.background = SpriteFactory.create_background(self.all_sprites, holiday)
        self.flyer = SpriteFactory.create_flyer(self.all_sprites, holiday)
    ...
    def display_score(self):
        ...
        score_surf = self.font.render(str(score.get_through_seasons_score() + score.get_score()), True, 'black')
        ...
        while running:
            ...
            for event in pygame.event.get():
                ...
                if event.type == self.obstacle_timer:
                    SpriteFactory.create_obstacle([self.all_sprites, self.collision_sprites], self.selected_mode)
